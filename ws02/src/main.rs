...
fn main() {
    ...
    hm.insert(String::from("Tom"), 99);
    hm.insert(String::from("Shrey"), 88);
    
    // String
    // &str
    // do explict casting for types

    // Option<i32>
    // Some(i32)
    // None
    println!("{:?}", hm.get("Tom").expect(""));
    
    // macro: Copy, Clone, Debug
    
    // Copy
    let i = 42;
    let i2 = i.clone(); // Copy construction, bit by bit copy
    println!("{}", i);
    
    // If String can use Copy, what will happen?
    // both string s1 and s2 would have a pointer
    // pointing towards the same addr, which will
    // result double free issue
    
    // Clone

    let s = String::from("Hello");
    // let s2 = s; // Move construction
    let s2 = s.clone(); // Clone operation, deep copy
    println!("{}", s);
